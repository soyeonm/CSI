...
class PermCLR(object):
	...
	def inference(self, train_datasets, test_datasets, test_loaders):
		...
		for batch_i, batch_dict_tuple in enumerate(zip(*test_loaders)): 
			...
			#chosen training
			chosen_train_dataset_tuples = [train_datasets[i][chosens[i]].unsqueeze(0) for i in range(len(train_datasets))] 
			...
			#pickle.dump(batch_dict_tuple, open('batch_dict_tuple.p', 'wb'))
