...
def get_dataset(P, dataset, test_only=False, image_size=None, download=False, eval=False):
    if not(P.resize_cifar) and dataset in ['imagenet', 'cub', 'stanford_dogs', 'flowers102',
        ...
        if eval:
            ...
            train_transform, test_transform = get_transform_imagenet() #putco3d small here
    else:
    ...
    #Original Imagenet-30 resized by me into 32x32
    elif dataset == 'imagenet_test_resized_by_me':
        assert test_only and image_size is not None
...
def get_superclass_list(dataset):
    ...
    elif dataset == 'co3d_small':
        return CO3D_SMALL_SUPERCLASS
    else:
        raise NotImplementedError()
